...
from src.chapter4 import (max_square_plot_size, mergesort, quicksort,
                       quicksort_hoare, quicksort_lomuto)
...
sorting_test_cases = list(flatten([[(list(p), sorted(p)) for p in permutations(range(x))] for x in range(3)]))
