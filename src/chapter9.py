...
def optimal_pick(items, knapsack_capacity) -> list:
    ...
    for i, item in enumerate(items):
        for j in range(knapsack_capacity):
            ...
            previous_max_value, previous_max_items = (0, set()) if previous_max_row < 0 else (grid[previous_max_row][j][0], grid[previous_max_row][j][1])
            remaining_space_column = j - item["mass"]
            current_item_and_remaining_space_value = 0 if item["mass"] - 1 > j else (
                item["price"] + (0 if remaining_space_column < 0 else grid[previous_max_row][remaining_space_column][0])
            )
            current_item_and_remaining_space_items = set() if item["mass"] - 1 > j else (
                set([item["name"]]).union(set() if remaining_space_column < 0 else grid[previous_max_row][remaining_space_column][1])
            )
            if previous_max_value > current_item_and_remaining_space_value:
                grid[i][j] = [previous_max_value,previous_max_items]
            else:
                grid[i][j] = [current_item_and_remaining_space_value, current_item_and_remaining_space_items]
    return grid[i][j]
...
def longest_common_substring(a: str, b: str) -> str:
    cell = [[0 for _ in range(len(a)+2)] for _ in range(len(b)+2)] # padding of size 1
    ...
    return b[longest_substring_coords[0]-longest_substring_length:longest_substring_coords[0]]
