...
def greedy_pick_price_per_mass(
    items: list[dict[str, str | int]], knapsack_capacity: int
) -> int:
    total_value: int = 0
...
def set_covering(stations, states_to_cover) -> set[str]:
    ...
    while states_to_cover:
        ...
        stations_and_new_states_covered_by_station: list[tuple[str, set[str]]] = sorted(
            (
                (station, states_to_cover.intersection(states_for_station))
        ...
        (
            best_station,
            states_covered_by_best_station,
        ) = stations_and_new_states_covered_by_station[-1]
